...
/**
 * Class WP_EXT_Fursona_Theme
 * ------------------------------------------------------------------------------------------------------------------ */

class WP_EXT_Fursona_Theme extends WP_EXT_Fursona {
	...
	public function enqueue_style() {
		if ( ! is_admin() && ( is_singular( $this->pt_ID )
		                       ...
		                       || is_active_widget( 0, 0, 'wp-ext-' . $this->domain_ID ) ) ) {
 ...
 *
 * @return WP_EXT_Fursona_Theme
 * ------------------------------------------------------------------------------------------------------------------ */

function WP_EXT_Fursona_Theme() {
	static $object;

	if ( null == $object ) {
		$object = new WP_EXT_Fursona_Theme;
	}
...
add_action( 'plugins_loaded', [ WP_EXT_Fursona_Theme(), 'run' ] );
